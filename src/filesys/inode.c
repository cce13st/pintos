  ...
  if (pos < inode->data.length)
	{
		...
		i %= 200;
		uint16_t imap[256];
		disk_read (filesys_disk, inode->data.index[j], imap);
		return imap[i];
	}
  else
    return -1;
      ...
      /*if (free_map_allocate (sectors, &disk_inode->start))
        {
          ...
          success = true; 
			...
			for (i=0; i<sectors; i++){
				...
				else if (sectors-i > 256)
				{	
					uint16_t indirect, imap[256];
					int j;
					for (j=0; j<256; j++){
						free_map_allocate (1, imap+j);
						disk_write (filesys_disk, imap[j], zeros);
					}
					free_map_allocate (1, &indirect);
					disk_write (filesys_disk, indirect, imap);
					disk_inode->index[i] = indirect;
				}
				else
				{
					uint16_t indirect, imap[256];
					int j;
					for (j=0; j<sectors-199; j++){
						free_map_allocate (1, imap+j);
						disk_write (filesys_disk, imap[j], zeros);
					}
					free_map_allocate (1, &indirect);
                ...
                off_t offset) 
{
  ...
  if (inode->deny_write_cnt)
	...
	if (inode->data.length < offset+size)
	{
		...
		{
			int add = bytes_to_sectors (extend);
